...
Python パッケージに加え、ブラウザ録音の後処理に `/usr/bin/ffmpeg` を使用します。
FFmpeg が利用できない場合も録音自体は継続し、ブラウザ側処理済み音声へフォールバックします。
...
ブラウザ録音でノイズ除去が ON の場合、Chrome の NS/AGC（EC は OFF）、Web Audio の音声帯域フィルター、
FFmpeg の保守的な FFT ノイズ低減と動的レベル正規化を順に適用します。
