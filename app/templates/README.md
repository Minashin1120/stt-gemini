...
3. モバイル Chrome のマイク制約は歴史的に挙動が変わりやすい。OFFは取得後の `applyConstraints` ではなく、初回 `getUserMedia` の exact 制約で録音プリセットを決めること。ただし `buildMicConstraintAttempts` は OFF 時に「EC単独の exact:false → NS単独の exact:false → 全項目の exact:false → relaxed」の順で試すこと（全項目まとめての exact:false を最初に試すと、2024年以降の Chrome/Android で reject され OFF が効かなくなる）
