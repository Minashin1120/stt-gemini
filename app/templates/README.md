...
- **録音タブ**: `getUserMedia`、マイクの事前取得・検証と録音開始時の再利用（ノイズ除去スイッチ変更時は自動的に再準備）、モバイルの内蔵マイク `deviceId` 厳密指定、初回取得時の必須音声処理制約、AudioWorklet録音、波形・実入力レベル、MP3（lamejs）/ 真の PCM WAV エンコード
